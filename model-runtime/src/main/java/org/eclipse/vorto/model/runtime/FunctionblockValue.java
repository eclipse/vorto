...
public class FunctionblockValue implements IValidatable {
	...
	public ValidationReport validate() {
		...
		for (ModelProperty configProperty : meta.getConfigurationProperties()) {
			checkProperty(getConfiguration(), configProperty, meta.getId().getName().toLowerCase() + "", report);
		}
		return report;
	}
	...
	private static void checkPrimitiveTypeValue(String path, Object propertyValue, ModelProperty property,
		...
		/*
		 * The below check for integer being a double is added due to a bug in JXpath
		 * which uses XPath 1.0, when any number (Integer/Double) is passed in XPath version 1.0, it is
		 * always passed as a Double
		 * (https://stackoverflow.com/questions/4721488/xpath-query-value-comparison-
		 * problem).
		 * (https://commons.apache.org/proper/commons-jxpath/).
		 * To bypass the above, whenever a number is given in Integer type, it will be checked for double,
		 * also it will be checked if its a whole number (whether it is a whole number like 1.0/2.0),
		 * if not then validation fails for values such as 1.1, 2.3 etc.
		 * 
		 */
