...
then
  ...
  # versioning the artifact in EBS
  echo "versioning the artifact at EBS"
  aws elasticbeanstalk create-application-version --application-name "vorto-dev-java-11" --no-auto-create-application --version-label "build-job_${ELASTIC_BEANSTALK_LABEL}_repo" --description "Build ${TRAVIS_JOB_NUMBER} - Git Revision ${TRAVIS_COMMIT_SHORT} for repository in dev" --source-bundle S3Bucket="$VORTO_S3_BUCKET",S3Key="${ARTIFACT_NAME}_${ELASTIC_BEANSTALK_LABEL}.jar"

  # updating environment in EBS
  echo "update Java 11 environment in EBS"
  aws elasticbeanstalk update-environment --application-name "vorto-dev-java-11" --environment-name "VortoDevJava11-env" --version-label "build-job_${ELASTIC_BEANSTALK_LABEL}_repo"
else
