...
/**
 * This interface deals with the data exchange between the client application
 * and the device. The data is compliant to the Vorto Information Model.
 */
public interface IDeviceData {

	/**
	 * Creates a new configuration to set properties. Send the configuration to
	 * device via
	 ...
	 *
	 * @param configuration
	 *            configuration to write to the device
	 * @param deviceId
	 *            deviceId of the device. See
	 *            {@link IDeviceDiscovery#listAvailableDevices(IDeviceDiscoveryCallback, int)}
	 *            } for device id discovery
	 * @throws {@link
	 *             DeviceConfigurationProblem}} if the given configuration
	 ...
	 *            } for device id discovery
	 * @param dataCallback
	 *            the callback handler when data is received by the device
	 */
	void receiveAsync(String infomodelProperty, String deviceId, IDataCallback dataCallback);

	/**
	 * Configuration Problem that may occur during writing configuration to the
	 * device
	 *
	 */
