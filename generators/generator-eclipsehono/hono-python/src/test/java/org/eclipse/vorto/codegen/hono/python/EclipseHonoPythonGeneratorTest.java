...
public class EclipseHonoPythonGeneratorTest extends AbstractGeneratorTest {
  ...
  public void checkAPIStatusPropertiesInFunctionBlock() throws Exception {
    IGenerationResult generationResult =
        pythonGenerator.generate(modelProvider(), InvocationContext.simpleInvocationContext());

    Generated generatedfile =
        zipFileReader(generationResult, "StatusPropertiesFunctionBlock", ".py");
    File defaultFile = new File(getClass().getClassLoader()
        .getResource("defaultFileFormat/StatusPropertiesFunctionBlock.py").toURI());
  ...
  public void checkAPIConfigPropertiesInFunctionBlock() throws Exception {
    ...
    Generated generatedfile =
        zipFileReader(generationResult, "ConfigPropertiesFunctionBlock", ".py");
    File defaultFile = new File(getClass().getClassLoader()
        .getResource("defaultFileFormat/ConfigPropertiesFunctionBlock.py").toURI());
    assertEquals(IOUtils.toString(FileUtils.openInputStream(defaultFile)),
        new String(generatedfile.getContent(), "utf-8"));
  ...
  public void testAPIFunctionBlockWithEvents() throws Exception {
    ...
    Generated generatedfile =
        zipFileReader(generationResult, "eventsAndOperationsFunctionBlock", ".py");
  ...
  public void testConnectorGeneratedMQTTClient()
    ...
    Generated generatedFileHonoMqttClient =
        zipFileReader(generationResult, "DittoSerializer", ".py");
    File defaultFileHonoMqttClient = new File(
        getClass().getClassLoader().getResource("defaultFileFormat/DittoSerializer.py").toURI());
  ...
  public void testClientAppGeneratedModels()
    ...
    File defaultFile =
        new File(getClass().getClassLoader().getResource("defaultFileFormat/MainApp.py").toURI());
    assertEquals(IOUtils.toString(FileUtils.openInputStream(defaultFile)),
        new String(generatedfile.getContent(), "utf-8"));
