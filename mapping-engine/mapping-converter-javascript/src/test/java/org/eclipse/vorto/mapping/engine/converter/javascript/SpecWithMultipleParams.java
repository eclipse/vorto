...
public class SpecWithMultipleParams extends AbstractTestSpec {
  ...
  private FunctionblockModel createButtonFb() {
    ...
    digitalInputCount
        .addStereotype(Stereotype.createWithXpath("button:convertArray(/data,/header)"));
  ...
  public FunctionLibrary getScriptFunctions(IScriptEvalProvider evalProvider) {
    ...
    evaluator.addScriptFunction(new ScriptClassFunction("convertArray",
        "function convertArray(arrayValue,header) {  return header+String.fromCharCode(arrayValue[0])}"));
