...
public class JsonMappingTest {
	...
	public void testMapWithSimpleCondition() throws Exception {
		IDataMapper mapper = new DataMapperBuilder().withSpecification(new SpecWithCondition())
				.build();

		String json = "{\"count\" : 2 }";

		InfomodelValue mappedOutput = mapper.mapSource(gson.fromJson(json, Object.class));
		assertFalse(mappedOutput.get("button").getStatusProperty("sensor_value") != null);
		assertEquals(2.0, mappedOutput.get("button").getStatusProperty("sensor_value2").getValue());

		json = "{\"count\" : 0 }";

		mappedOutput = mapper.mapSource(gson.fromJson(json, Object.class));
		assertEquals(0.0, mappedOutput.get("button").getStatusProperty("sensor_value").getValue());
		assertFalse(mappedOutput.get("button").getStatusProperty("sensor_value2") != null);
	}
	...
	public void testMapWithJxpathCondition() throws Exception {
		IDataMapper mapper = new DataMapperBuilder().withSpecification(new SpecWithConditionXpath())
				.build();

		String json = "{\"data\" : [{\"id\": 100,\"value\": \"x\"},{\"id\": 200,\"value\": \"y\"}]}";

		InfomodelValue mappedOutput = mapper.mapSource(gson.fromJson(json, Object.class));
		assertEquals(100.0, mappedOutput.get("button").getStatusProperty("sensor_value").getValue());
	...
	public void testMappingUsingListInput() throws Exception {
		...
		IDataMapper mapper = new DataMapperBuilder().withSpecification(new SpecWithArrayPayload())
				.build();
		...
		assertEquals("DOUBLE", buttonFunctionblockData.getStatusProperty("sensor_value").getValue());
	...
	public void testMapSingleFunctionblockOfInfomodel2() {
		IDataMapper mapper = new DataMapperBuilder().withSpecification(new SpecWithConditionedRules())
				.build();
		...
		assertEquals("Locked", (String)doorStateFunctionblockData.getStatusProperty("sensor_value").getValue());
	...
	public void testMappingWithInfoModelUsingSameFunctionblock() throws Exception {
		...
		IDataMapper mapper = new DataMapperBuilder().withSpecification(new SpecWithSameFunctionblock())
				.build();
		...
		assertEquals(2.0, buttonFunctionblockData.getStatusProperty("sensor_value").getValue());

		FunctionblockValue button2FunctionblockData = mappedOutput.get("btn2");

		assertEquals(10.0, button2FunctionblockData.getStatusProperty("sensor_value").getValue());
