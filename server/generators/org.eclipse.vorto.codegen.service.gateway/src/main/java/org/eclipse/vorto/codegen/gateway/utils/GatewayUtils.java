...
public class GatewayUtils {
	...
	private static final String[] PROPERTIES = {"tags", "classifier", "name", "image32x32", "image144x144", 
			"documentationUrl", "description", "creator"};
	
	private static final String VORTO_SERVICE_TAGS = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[0]);
	private static final String VORTO_SERVICE_CLASSIFIER = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[1]);
	private static final String VORTO_SERVICE_NAME = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[2]);
	private static final String VORTO_SERVICE_IMAGE32X32 = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[3]);
	private static final String VORTO_SERVICE_IMAGE144X144 = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[4]);
	private static final String VORTO_SERVICE_DOCUMENTATION_URL = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[5]);
	private static final String VORTO_SERVICE_DESCRIPTION = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[6]);
	private static final String VORTO_SERVICE_CREATOR = String.format(FILE_PROPERTY_PREFIX, PROPERTIES[7]);
		...
		return gen -> {
			Stream.of(PROPERTIES).forEach(property -> {
				...
				if (env.containsProperty(modProperty)) {
					...
					LOGGER.info(String.format("Overriding %s for %s with %s", property, propertyKey, modValue));
