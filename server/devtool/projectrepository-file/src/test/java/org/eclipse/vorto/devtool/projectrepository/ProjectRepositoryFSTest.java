...
public class ProjectRepositoryFSTest {
	...
	public void createProjectWithProperties() {
		...
		ProjectResource project = repoService.createProject("myProject", properties,
	...
	public void changeProjectProperties() {
		...
		project = (ProjectResource) repoService.createQuery().name("myProject").type(ResourceType.ProjectResource)
				.singleResult();
	...
	public void queryAndDeleteProject() {
		...
		ProjectResource project = repoService.createProject(projectName, "JUnit test for creating a project");
	...
	public void addMultipleFilesAndFoldersToProject() {
		...
		FileUploadHandle handle1 = new FileUploadHandle(project, "someFile", "content".getBytes(), null);
		FileUploadHandle handle2 = new FileUploadHandle(project, "someOtherFile", "content".getBytes(), null);
		FolderUploadHandle handle3 = new FolderUploadHandle(project, "someFolder");

		repoService.uploadResource("someComment", handle1, handle2, handle3);

		List<Resource> results = repoService.createQuery().pathLike("myProject").list();
	...
	public void addFolderToProject() {
		...
		FolderUploadHandle handle = new FolderUploadHandle(project, "someFolder");
		repoService.uploadResource("someComment", handle);

		Resource result = repoService.createQuery().path("someProject/someFolder").singleResult();
	...
	public void addFileWithPropertiesToProject() {
		...
		FileUploadHandle handle = new FileUploadHandle(project, "someFile", "content".getBytes(), null);
		Map<String, String> properties = new HashMap<String, String>();
		properties.put(Resource.META_PROPERTY_AUTHOR, "alex");
		handle.setProperties(properties);
		repoService.uploadResource("someComment", handle);

		Resource result = repoService.createQuery().path("someProject/someFile").singleResult();
		assertEquals(ResourceType.FileResource, result.getType());
		assertEquals(2, result.getProperties().size());
		ResourceContent content = repoService.getResourceContent((FileResource) result);
	...
	public void addFilesWithPropertiesToProject() {
		...
		FileUploadHandle handle2 = new FileUploadHandle(project, "anotherFile", "content".getBytes(), null);
		...
		result = repoService.createQuery().path("someProject/anotherFile").singleResult();
		assertEquals(FileResource.class, result.getClass());

		content = repoService.getResourceContent((FileResource) result);
		assertTrue(Arrays.equals("content".getBytes(), content.getContent()));
	...
	public void removeFolderFromProject() {
		...
		Resource resource = repoService.createQuery().path("someProject").nameLike("someFolder").singleResult();
		repoService.deleteResource(resource);
		assertNull(repoService.createQuery().path("someProject").nameLike("someFolder").singleResult());
	}
	...
	public void removeFileFromProject() {
		...
		Resource resource = repoService.createQuery().path(testFolderName).nameLike(testFileName).singleResult();
		repoService.deleteResource(resource);
		assertFalse(new File(System.getProperty("user.dir") + "/projects/someProject/file1.meta.props").exists());
		assertNull(repoService.createQuery().path("removeFileFromProject").nameLike("removeFileFromProjectFile")
				.singleResult());
	}
	...
	public void removeProject() {
		...
		Resource resource = repoService.createQuery().nameLike("someProject").singleResult();
		repoService.deleteResource(resource);
	...
	public void getResourceContentExistant() {
		...
		FileUploadHandle handle = new FileUploadHandle(project, "someFile", "content".getBytes());
		repoService.uploadResource("someComment", handle);

		ResourceContent content = repoService
				.getResourceContent((FileResource) repoService.createQuery().path("myProject/someFile").singleResult());
		assertTrue(Arrays.equals("content".getBytes(), content.getContent()));
	}
	...
	public void queryPathSingleResult() {
		...
		Resource resource = repoService.createQuery().path("queryPathTestProject/queryPathTestFolder").singleResult();

		assertEquals("queryPathTestProject" + "/" + "queryPathTestFolder", resource.getPath());
	}
	...
	public void queryPathLikeMultipleResults() {
		...
		createTestFolder(project, "queryPathTestFolder" + "/queryPathTestSubFolder1");
		createTestFolder(project, "queryPathTestFolder" + "/queryPathTestSubFolder2");
		createTestFolder(project, "queryPathTestFolder" + "/queryPathTestSubFolder3");

		List<Resource> resources = repoService.createQuery().pathLike("queryPathTestProject/queryPathTestFolder")
				.list();
	...
	public void queryResourcePathAndName() {
		...
		Resource resource = repoService.createQuery().pathLike("someProject").nameLike("someFolder").singleResult();
	...
	private FolderResource createTestFolder(ProjectResource project, String path) {
		...
		repoService.uploadResource("commitComment", new FolderUploadHandle(project, path));
	...
	private void createTestFolder(FolderResource resource, String path, String author) {
		...
		repoService.uploadResource("commitComment", handle);
	}
	...
	public void queryFolderResource() {
		...
		List<Resource> resource = repoService.createQuery().path("queryFolderResourceTest/folder").list();
	...
	public void queryFileResource() {
		...
		List<Resource> resource = repoService.createQuery().path("queryFileResourceTest/file").list();
	...
	private void createTestFile(FolderResource folderResource, String path) {
		FileUploadHandle handle = new FileUploadHandle(folderResource, path, "content".getBytes(), null);
		repoService.uploadResource("someComment", handle);
	}
	...
	public void queryLastModifiedAndPath() {
		...
		Resource resource1 = repoService.createQuery().path("queryLastModified/file").singleResult();
		Resource resource2 = repoService.createQuery().lastModified(resource1.getLastModified())
				.pathLike("queryLastModified/file").singleResult();
		assertEquals(resource1.getLastModified().toString(), resource2.getLastModified().toString());
	}
	...
	public void queryByTypeFolder() {
		...
		Resource resource = repoService.createQuery().type(ResourceType.FolderResource).singleResult();
		assertEquals(FolderResource.class, resource.getClass());
	...
	public void queryByTypeFile() {
		...
		Resource resource = repoService.createQuery().type(ResourceType.FileResource).singleResult();
	...
	public void queryByTypeProject() {
		...
		Resource resource = repoService.createQuery().type(ResourceType.ProjectResource).singleResult();
	...
	public void queryByTypeProjectMultipleResults() {
		...
		List<Resource> resource = repoService.createQuery().type(ResourceType.ProjectResource).list();
	...
	public void queryAllResourcesOfProject() {
		...
		List<Resource> resource = repoService.createQuery().pathLike("myproject").list();
	...
	public void queryByAuthor() {
		...
		List<Resource> resource = repoService.createQuery().author("admin").list();
		assertEquals(3, resource.size());
	}
	...
	public void queryByAuthorVersionTypeAndName() {
		...
		createTestFile(project, "file");
		repoService.createVersionOfProject("testVersion", "queryByTypeProject1");
		repoService.createVersionOfProject("testVersion2", "queryByTypeProject3");
		repoService.createVersionOfProject("testVersion2", "queryByTypeProject1");

		List<Resource> resource = repoService.createQuery().author("admin").type(ResourceType.FileResource)
				.version("testVersion").name("file").list();
	...
	public void queryByAuthorTypeAndNameWithoutVersion() {
		...
		List<Resource> resource = repoService.createQuery().author("admin").type(ResourceType.FileResource).name("file")
				.list();
		assertEquals(2, resource.size());
	...
	public void createVersion() {
		...
		Resource resource = repoService.createQuery().version("testVersion").singleResult();
	...
	public void createVersionOfTwoProjects() {
		...
		List<Resource> resource = repoService.createQuery().version("testVersion").list();
		assertEquals(2, resource.size());
	}
	...
	public void tagFolder() {
		...
		Resource resource = repoService.createQuery().path("tagTest/folder").singleResult();
		repoService.tag(resource, "testTag");
		resource = repoService.createQuery().path("tagTest/folder").singleResult();
		assertEquals("vr:testTag", resource.getTags()[0]);
	}
	...
	public void tagFile() {
		...
		Resource resource = repoService.createQuery().path("tagTest/file").singleResult();
		repoService.tag(resource, "testTag");
		resource = repoService.createQuery().path("tagTest/file").singleResult();
		assertEquals("vr:testTag", resource.getTags()[0]);
	}
	...
	protected ProjectResource createTestProject(String name) {
		return repoService.createProject(name, "JUnit test for creating a project");
	}

	protected ProjectResource createTestProject(String name, String author) {
		Map<String, String> properties = new HashMap<String, String>();
		properties.put(Resource.META_PROPERTY_AUTHOR, author);
		return repoService.createProject(name, properties, "JUnit test for creating a project");
	}
}
