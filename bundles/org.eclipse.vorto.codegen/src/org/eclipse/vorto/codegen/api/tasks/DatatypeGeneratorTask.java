...
/**
 * Generator task that traverses all types used in an information model and generates their platform specific representations
 * 
 * @author Alexander Edelmann - Initial commit
 *
 */
public class DatatypeGeneratorTask implements ICodeGeneratorTask<InformationModel> {
	...
	public DatatypeGeneratorTask(IFileTemplate<Entity> entityTemplate, IFileTemplate<org.eclipse.vorto.core.api.model.datatype.Enum> enumTemplate) {
		this.entityTemplate = entityTemplate;
		this.enumTemplate = enumTemplate;
	}
	
	@Override
	public void generate(InformationModel ctx, IOutputter outputter) {
		...
		for (Type type : allTypesUsedInModel) {
			if (type instanceof Entity) {
				outputter.output(new Generated(entityTemplate.getFileName((Entity)type),entityTemplate.getPath((Entity)type),entityTemplate.getContent((Entity)type)));
			} else if (type instanceof Enum) {
				outputter.output(new Generated(enumTemplate.getFileName((Enum)type),enumTemplate.getPath((Enum)type),enumTemplate.getContent((Enum)type)));
			}
		}
	}
