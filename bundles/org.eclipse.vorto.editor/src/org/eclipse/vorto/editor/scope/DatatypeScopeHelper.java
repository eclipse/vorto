...
/**
 * Construct local scope for cross reference type. This is used when convert
 * from xmi to dsl, xmi does not contain referecing EObject type. However we can
 * constuct the cross reference based on type href info e.g.
 * "using|com.bosch.Color"
 * 
 */
public class DatatypeScopeHelper {

	public static IScope getDataTypeReferenceScope(
			IQualifiedNameConverter converter, EObject context,
			EReference reference) {
		IScope scope = IScope.NULLSCOPE;

		if (context instanceof ObjectPropertyType
				&& DatatypeScopeHelper
						.isFromVirtualFile((ObjectPropertyType) context)) {
	...
	public static IScope getObjectPropertyScope(
	...
	private static IScope getTypeScope(IQualifiedNameConverter converter,
			Type type, EReference reference) {
		List<IEObjectDescription> objects = new ArrayList<IEObjectDescription>();

		String objectPropertyTypeName = getObjectPropertyNameFromProxyUri(((TypeImpl) type)
				.eProxyURI().toString());

		objects.add(EObjectDescription.create(
				converter.toQualifiedName(objectPropertyTypeName), reference));
		return new ProxyUriReferenceBasedScope(IScope.NULLSCOPE, objects);
	}

	public static boolean isFromVirtualFile(
			ObjectPropertyType objectPropertyType) {
		return objectPropertyType.eResource().getURI().toString()
				.contains("NON_EXIST_DSL_FILE");
	}
