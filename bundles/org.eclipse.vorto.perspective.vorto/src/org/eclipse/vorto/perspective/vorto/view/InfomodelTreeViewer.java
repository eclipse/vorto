	...
	protected void initContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
					...
					if (model.getId().getModelType() == ModelType.InformationModel) {
						...
						menuMgr.add(new ProjectAction("New Mapping Model",ImageUtil.getImage("add_obj.gif"),treeViewer.getLocalModelWorkspace()) {
							...
							public void doAction() {
								IModelElement firstElement = (IModelElement) treeViewer.getStructuredSelection()
										.getFirstElement();
								ModelId modelId = firstElement.getId();
								WizardDialog dialog = new WizardDialog(
										PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
										new MappingModelWizard(treeViewer.getLocalModelWorkspace().getProjectBrowser()
												.getSelectedProject(), modelId));
								dialog.create();
								dialog.open();
							}
						});
				...
				manager.add(new ProjectAction("New Information Model", ImageUtil.getImage("add_obj.gif"),
