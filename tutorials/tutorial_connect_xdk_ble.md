...
### 6. Short primer on the Virtual XDK Firmware
...
The sensors value stream is started by writing '00' on UUID handle 0x0039. You can then read the continues stream of sensor values at UUID handle 0x0034 and 0x0036. The 0x0034 contains values for 
either a computed orientation or accelerometer and gyrometer. This behaviour is controlled by the flag that we asked you to modify on Step #5. The 0x0036 can be two streams. If it starts with 01,
...
### 7. Read temperature from Bluetooth
...
- Create a new package in the Kura bundle, name it _com.example.kura.device_. We will put classes that implements reading from the device here.
