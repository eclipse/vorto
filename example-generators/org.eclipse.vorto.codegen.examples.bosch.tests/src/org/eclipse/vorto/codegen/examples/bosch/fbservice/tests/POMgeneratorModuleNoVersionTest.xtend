...
class POMgeneratorModuleNoVersionTest {
	
	private FunctionblockModel model;
	private POMTemplate pomgen;

	@Test
	def testGenerationWithNullVersion(){
		model = populateFBmodel();
		
		pomgen = new POMTemplate(null)
		...
		@Test
	def testGenerationWithEmptyVersion(){
		model = populateFBmodel();
		
		pomgen = new POMTemplate(" ")
...
<properties>
      <msp.osgi.import.pkg>*,com.bosch.functionblock.demo.fridge._3</msp.osgi.import.pkg>
      <m2m.version>LATEST</m2m.version>
