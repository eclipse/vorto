...
public class TestInforModelFactory {
	public static InformationModel createInformationModel() {
		...
		informationModel.setDisplayname("Lighting Device");
		...
		informationModel.getProperties().add(
				createPropertyFromFunctionBlockModel("fbm1",
						TestFunctionblockModelFactory
								.createFBmodelWithProperties()));
		informationModel.getProperties().add(
				createPropertyFromFunctionBlockModel("fbm2",
						TestFunctionblockModelFactory.createFBmodel()));
		return informationModel;
	}

	private static FunctionblockProperty createPropertyFromFunctionBlockModel(
			String name, FunctionblockModel fbm) {
