...
begin
    ...
    declare authentication_provider_count int;
    declare technical_user_count int;
    declare authentication_provider_id_count int;
    declare subject_count int;

    # checks if the authentication_provider column exists, and deletes it if it does
    set authentication_provider_count = (
        select count(*) from information_schema.COLUMNS where TABLE_NAME='user' and COLUMN_NAME='authentication_provider'
    );
    if (authentication_provider_count > 0) then
        ALTER TABLE user DROP COLUMN authentication_provider;
    end if;
