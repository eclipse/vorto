...
@RunWith(SpringJUnit4ClassRunner.class) @ActiveProfiles(profiles = {"local-ui-test"})
...
public abstract class AbstractUITest {
    ...
    @Autowired protected AuthenticationProviderMock mock;

    @Autowired protected UserRepository userRepository;

    @Rule public BrowserWebDriverContainer chrome = new BrowserWebDriverContainer()
        .withRecordingMode(BrowserWebDriverContainer.VncRecordingMode.RECORD_FAILING,
            new File("./target/")).withCapabilities(DesiredCapabilities.chrome());

    @BeforeClass public static void configureOAuthConfiguration() {
    ...
    @Before public void prepareTests() throws Exception {
        setRootUrl();
        setUpTest();
    ...
    public void testCreateNamespace(CreateNamespaceParams... params) {
        ...
        this.seleniumVortoHelper.getRemoteWebDriver().findElementByXPath(String
            .format("//td/div[@class='ng-binding' and contains(.,'%s')]",
    ...
    public CreateModelResultHandler createModel(CreateModelParams... params) {
        ...
        remoteWebDriver.findElementByXPath(
            String.format("//input[@name='modelType' and @value='%s']", usedParams.getType()))
            .click();
        remoteWebDriver.findElementByXPath("//button[contains(@ng-click,'next(')]").click();
        Select namespaceComboBox =
    ...
    public void loadModelDetailsUI(ModelId id) {
        ...
        waitForDetailsLoading.until(ExpectedConditions.visibilityOf(
