...
public class ModelRepositoryControllerTest extends AbstractIntegrationTest {
  ...
  public void saveModel() throws Exception {
    ...
    try {
      repositoryServer
          .perform(delete("/rest/tenants/playground/models/" + testModelId).with(userAdmin));
      repositoryServer
          .perform(delete("/rest/tenants/playground/models/" + locationModelId).with(userAdmin));
    } catch (Exception e) {
    ...
    this.repositoryServer
        .perform(put("/rest/tenants/playground/models/" + testModelId)
    ...
    this.repositoryServer
        .perform(put("/rest/tenants/playground/models/com.test1:TrackinDevice:0.0.1")
            .contentType(MediaType.APPLICATION_JSON).content(json).with(userAdmin))
  ...
  public void createVersionOfModel() throws Exception {
    ...
    repositoryServer
        .perform(post("/rest/tenants/playground/models/" + testModel.prettyName + "/versions/1.0.1")
            .with(userAdmin).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isCreated());
    repositoryServer
        .perform(post("/rest/tenants/playground/models/com.test1:Location:1.0.0/versions/1.0.1")
            .with(userAdmin).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound());
    repositoryServer
        .perform(delete("/rest/tenants/playground/models/" + testModel.prettyName).with(userAdmin));
  ...
  public void deleteModelResource() throws Exception {
    ...
    repositoryServer.perform(delete("/rest/tenants/playground/models/" + modelId).with(userAdmin))
        .andExpect(status().isOk());
    this.repositoryServer
        .perform(put("/rest/tenants/playground/models/" + modelId)
            .contentType(MediaType.APPLICATION_JSON).content(json).with(userAdmin))
        .andExpect(status().isNotFound());

    // delete non existent model
    repositoryServer
        .perform(delete("/rest/tenants/playground/models/com.test:ASDASD:0.0.1").with(userAdmin))
        .andExpect(status().isNotFound());
  }
  ...
  public void getUserModels() throws Exception {
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/mine/download").with(userAdmin))
        .andExpect(status().isOk());
  }
  ...
  public void downloadMappingsForPlatform() throws Exception {
    this.repositoryServer.perform(
        ...
        .andExpect(status().isOk());
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/com.test:Test1:1.0.0/download/mappings/test")
            .with(userAdmin))
        .andExpect(status().isNotFound());
  }
  ...
  public void runDiagnostics() throws Exception {
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/" + testModel.prettyName + "/diagnostics")
            .with(userAdmin))
        .andExpect(status().isOk());
    this.repositoryServer
        .perform(
            get("/rest/tenants/playground/models/test:Test123:1.0.0/diagnostics").with(userAdmin))
        .andExpect(status().isNotFound());
  }
  ...
  public void getPolicies() throws Exception {
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/" + testModel.prettyName + "/policies")
            .with(userAdmin))
        .andExpect(status().isOk());
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/" + testModel.prettyName + "/policies")
  ...
  public void getUserPolicy() throws Exception {
    this.repositoryServer.perform(
        ...
        .andExpect(status().isNotFound());
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/" + testModel.prettyName + "/policy")
            .with(userCreator))
        .andExpect(status().isOk());
  }
  ...
  public void addValidPolicyEntry() throws Exception {
    ...
    this.repositoryServer
        .perform(put("/rest/tenants/playground/models/" + testModel.prettyName + "/policies")
            .contentType(MediaType.APPLICATION_JSON).content(json).with(userAdmin))
        .andExpect(status().isOk());
    this.repositoryServer
        .perform(get("/rest/tenants/playground/models/" + testModel.prettyName + "/policy")
