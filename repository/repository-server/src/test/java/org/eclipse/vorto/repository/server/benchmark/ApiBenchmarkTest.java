 ...
 *
 * If you want to execute the test directly in your IDE you need to:
 * <ul>
 *   <li>
 *      Add {@code clean resources:testResources jmeter:configure -Pbenchmark-tests}
 *   </li>
 *   <li>
 *     Set the working directory to {@literal [root]/vorto/repository/repository-server}
 *   </li>
 * </ul>
...
@ContextConfiguration(initializers = ApiBenchmarkTest.Initializer.class, classes = {
    VortoRepository.class, ApiBenchmarkTest.ModeshapeTestConfiguration.class})
public class ApiBenchmarkTest {
  ...
  public static JdbcDatabaseContainer mySQLContainer = (JdbcDatabaseContainer) new CustomMysqlTestContainer(
      "mysql:5.6.44").withDatabaseName("vorto_schema").withPassword("vorto").withUsername("vorto")
  ...
  private static void globalJMeterConfiguration() throws URISyntaxException, IOException {
    BENCHMARK_PATH_STRING = Paths
        .get(ApiBenchmarkTest.class.getClassLoader().getResource(JMETER_HOME_DIR).toURI())
        .toString();
  ...
  private static String determineJMeterHome() throws URISyntaxException, IOException {
    try {
      Path jmeterPath = Files.list(Paths.get(BENCHMARK_PATH_STRING))
          .filter(path -> path.toFile().isDirectory() && !path.toString().contains("report"))
          .findAny().orElse(null);
  ...
  private void configureJMeter() throws IOException, URISyntaxException {
    ...
    JMeterUtils
        .setProperty(VORTO_RESULT_FILE_TREE_PROPERTY, BENCHMARK_PATH_STRING + "/resulttree.csv");
    // set result summary file path
    JMeterUtils.setProperty(VORTO_RESULT_FILE_SUMMARY_PROPERTY,
        BENCHMARK_PATH_STRING + "/resultsummary.csv");
    // load the test plan
    testPlanTree = SaveService.loadTree(Paths.get(ApiBenchmarkTest.class.getClassLoader()
        .getResource(JMETER_HOME_DIR + "/apiCallGetModel.jmx").toURI()).toFile());
  ...
  public static class ModeshapeTestConfiguration {
    ...
    public org.modeshape.jcr.RepositoryConfiguration repoConfiguration() throws Exception {
      ...
      editor
          .setString(RepositoryConfiguration.FieldName.CONNECTION_URL, mySQLContainer.getJdbcUrl());
  ...
  public void setUpTest() throws IOException, URISyntaxException {
    ...
    mock.setAuthorityListForUser(SpringUserUtils.toAuthorityList(
        Sets.newHashSet(DEFAULT_NAMESPACE_ROLES[0], DEFAULT_NAMESPACE_ROLES[5],
            DEFAULT_NAMESPACE_ROLES[1], DEFAULT_NAMESPACE_ROLES[2], DEFAULT_NAMESPACE_ROLES[3],
            DEFAULT_NAMESPACE_ROLES[4], DEFAULT_REPOSITORY_ROLES[0])), "user1");
    configureJMeter();
  }
  ...
  public void testGetModelApi() {
    ...
    if (!jUnitTestResultCollector.getFailedSamplesSet().isEmpty()) {
      Assert.fail(
          "Test failed because at least one instance of the following JMeter samples failed: "
              + jUnitTestResultCollector.getFailedSamplesSet().toString());
    }
  }
  ...
  public void createHtmlReport() throws IOException {
    ...
    HtmlReportGenerator generator = new HtmlReportGenerator(
        JMeterUtils.getProperty(VORTO_RESULT_FILE_SUMMARY_PROPERTY),
        BENCHMARK_PATH_STRING + "/user.properties", BENCHMARK_PATH_STRING + "/report");
  ...
  public static class Initializer implements
    ...
    private void loadTestData() {
      try {
        mySQLContainer
            .copyFileToContainer(MountableFile.forClasspathResource("VortoTestcontainer3.tar.xz"),
                "VortoTestcontainer3.tar.xz");
        ...
        mySQLContainer.execInContainer("bash", "-c",
            "usr/bin/mysql -uvorto -pvorto vorto_schema < VortoTestcontainer3.sql");
  ...
  private void renameJar() throws IOException {
    ...
    Path jmeterJar = Files.list(Paths.get(jMeterBin))
        .filter(fileName -> fileName.toString().contains("ApacheJMeter")).findAny().orElse(null);
