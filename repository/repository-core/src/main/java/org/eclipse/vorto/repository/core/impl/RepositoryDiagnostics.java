...
public class RepositoryDiagnostics {
	...
	public interface NodeDiagnostic extends Function<Node, Collection<Diagnostic>> { public String getName(); }
	...
	private Collection<Diagnostic> diagnoseNode(final Node node) throws RepositoryException {
		...
		for(NodeDiagnostic nodeDiagnosticTest : nodeDiagnosticTests) {
			try {
				...
				diagnostics.add(new Diagnostic(maybeModelId.orElse(null), "'" + nodeDiagnosticTest.getName() + "' returned an error '" + e.getMessage() + "'"));
			}
		}
