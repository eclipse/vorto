...
public class CiamUserStrategy extends AbstractVerifyAndIdStrategy {
	...
	public CiamUserStrategy(RestTemplate restTemplate, String publicKeyUri, IUserAccountService userAccountService,String clientId) {
		super(restTemplate, publicKeyUri, userAccountService);
		this.ciamClientId = Objects.requireNonNull(clientId);
	}
	
	/**
	 * Authenticates the user from the CIAM issued token by checking if the user is registered in the Repository  
	 */
	@Override
	public OAuth2Authentication createAuthentication(JwtToken accessToken) {
		...
		String userId = getUserId(tokenPayload).orElseThrow(() ->
		...
		UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(name.orElse(userId), "N/A",SpringUserUtils.toAuthorityList(user.getUserRoles()));
