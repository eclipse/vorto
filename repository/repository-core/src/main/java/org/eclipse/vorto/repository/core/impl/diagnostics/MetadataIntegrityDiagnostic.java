...
public class MetadataIntegrityDiagnostic implements NodeDiagnostic {
	...
	private Optional<Diagnostic> checkNodeProperty(final Node node, final String nodePropertyName, final String expectedValue) {
		...
		try {
			...
			if (!node.getProperty(nodePropertyName).getString().trim().equals(Strings.nullToEmpty(expectedValue).trim())) {
