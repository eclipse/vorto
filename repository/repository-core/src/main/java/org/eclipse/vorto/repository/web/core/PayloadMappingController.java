...
public class PayloadMappingController extends AbstractRepositoryController {
	...
	public Map<String, Object> createMappingSpecification(@PathVariable final String modelId,
		...
		if (!this.repository.getMappingModelsForTargetPlatform(ModelId.fromPrettyFormat(modelId), targetPlatform)
				.isEmpty()) {
	...
	public IMappingSpecification getMappingSpecification(@PathVariable final String modelId,
			@PathVariable String targetPlatform) throws Exception {
		org.eclipse.vorto.repository.core.ModelContent infoModelContent = modelController
				.getModelContentForTargetPlatform(modelId, targetPlatform);
		...
		for (ModelProperty fbProperty : infomodel.getFunctionblocks()) {
			...
			if (mappingId != null) {
				...
				org.eclipse.vorto.repository.core.ModelContent fbmContent = modelController
						.getModelContent(fbModelId.getPrettyFormat());
	...
	public FunctionblockModel getModelContentByModelAndMappingId(final @PathVariable String modelId,
		...
		org.eclipse.vorto.core.api.model.functionblock.FunctionblockModel fbm = (org.eclipse.vorto.core.api.model.functionblock.FunctionblockModel) mappingWorkspace
				.get().stream()
				.filter(model -> model instanceof org.eclipse.vorto.core.api.model.functionblock.FunctionblockModel)
				.findFirst().get();

		return ModelDtoFactory.createResource(fbm, Optional.of((MappingModel) mappingWorkspace.get().stream()
	...
	private FunctionblockModel initEmptyProperties(FunctionblockModel fbm) {
		...
		// adding empty stereotype for all configuration properties, if
		// necessary
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Successful download of mapping specification"),
			@ApiResponse(code = 400, message = "Wrong input"), @ApiResponse(code = 404, message = "Model not found") })
	@PreAuthorize("hasRole('ROLE_USER')")
	...
	public void downloadModelById(@PathVariable final String modelId, @PathVariable String targetPlatform,
		...
		response.setHeader(CONTENT_DISPOSITION, ATTACHMENT_FILENAME + modelID.getNamespace() + "_" + modelID.getName()
				+ "_" + modelID.getVersion() + "-mappingspec.json");
		response.setContentType(APPLICATION_OCTET_STREAM);
		try {
			MappingSpecification spec = (MappingSpecification) this.getMappingSpecification(modelId, targetPlatform);
			ObjectMapper mapper = new ObjectMapper();
			IOUtils.copy(new ByteArrayInputStream(mapper.writerWithDefaultPrettyPrinter().writeValueAsBytes(spec)),
