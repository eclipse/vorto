...
public class KeycloakUserStrategy extends AbstractVerifyAndIdStrategy {
	...
	/**
	 * Authenticates the user from the Keycloak issued token by checking if the user is registered in the Repository as well as reading the user's roles from the JWT Token 
	 */
	...
	public OAuth2Authentication createAuthentication(JwtToken accessToken) {
		...
		//FIXME: Use Spring Security Adapter for Keycloak to read roles from token and create authentication object
		...
		if (!resourceAccess.isPresent()) {
			throw new InvalidTokenException("Keycloak issued token does not define a property 'resource_access' containing roles.");
		}
		...
		if(roles != null){
			...
			authToken = new UsernamePasswordAuthenticationToken(name.orElse(userId), "N/A",SpringUserUtils.toAuthorityList(user.getUserRoles()));
		}
