...
public class ModelRepositoryDiagnosticsTest extends AbstractIntegrationTest {
	...
	public void testGetModelIdOnParentNode() {
		ModelId modelId = NodeDiagnosticUtils.getModelId("/com/ipso/smartobjects/load_control/1.1.0").get();
		assertEquals("load_control", modelId.getName());
		assertEquals("com.ipso.smartobjects", modelId.getNamespace());
		assertEquals("1.1.0", modelId.getVersion());
	}
	
	// TODO : This test doesn't actually work as the sequencer will rebuild the references everytime you restore the backup file, and so the references will 
	// correct themselves but I'm not deleting it as it shows how to code around the possible race problem when writing test that uploads a model then access
	// the metadata that still needs to be built by the sequencer. This shows how to execute code that actually waits for the sequencer
	// to finish.
	...
	public void withReferenceIntegrityProblems() {
		...
		try {
			modelRepository.getSession().getWorkspace().getObservationManager().addEventListener(new EventListener() {
