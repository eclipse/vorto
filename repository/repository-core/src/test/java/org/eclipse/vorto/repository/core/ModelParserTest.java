...
public class ModelParserTest extends AbstractIntegrationTest {
	...
	public void testModelParsing() {
		try {
			ModelInfo modelInfo = modelParserFactory.getParser("Color_encoding.type").parse(new ClassPathResource("sample_models/Color_encoding.type").getInputStream());
	...
	public void testModelParsingWithDependencyInRepo() {
		try {
			...
			try {
				ModelInfo modelInfo = modelParserFactory.getParser("ColorLightIM.infomodel").parse(new ClassPathResource("sample_models/ColorLightIM.infomodel").getInputStream());
				assertTrue(modelInfo != null);
	...
	public void testModelWithValidatorProblem() {
		try {
			modelParserFactory.getParser("Accelerometer-invalid.fbmodel").parse(new ClassPathResource("sample_models/Accelerometer-invalid.fbmodel").getInputStream());
