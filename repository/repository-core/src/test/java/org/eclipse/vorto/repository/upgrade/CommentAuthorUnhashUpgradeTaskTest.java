...
public class CommentAuthorUnhashUpgradeTaskTest {
	...
	private UsernamePasswordAuthenticationToken mockedAuthToken = Mockito.mock(UsernamePasswordAuthenticationToken.class);
	...
	public void testUserUpdateTask() {
		...
		commentsForEmantos.forEach(comment -> assertTrue(comment.getAuthor().equals(userContext2.getHashedUsername())));
		commentsForEmailPrefix.forEach(comment -> assertTrue(comment.getAuthor().equals(userContext.getHashedUsername())));
