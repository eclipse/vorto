...
define(["../init/appService"], function (repository) {
  var timeout = 30000;
  repository.factory('SessionTimeoutService',
      ['$rootScope', '$http', '$q', '$location', '$timeout',
        function ($rootScope, $http, $q, $location, $timeout) {
          return {
          ...
          function checkSessionTimeout($scope) {
            $http
            .get('rest/config/session/timeout')
            .then(
                function (result) {
                  var timeoutOffset;
          ...
          function countDown($scope, timeoutOffset) {
            ...
            $scope.sessionTimeoutCountdownStep = function () {
              $scope.sessionTimeoutCountdown = $scope.sessionTimeoutCountdown
                  - 1;
