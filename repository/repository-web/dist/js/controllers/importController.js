...
define(["../init/appController"], function (repositoryControllers) {
  ...
  repositoryControllers.controller('ImportController',
        ...
        function ($scope, $rootScope, $http, $location) {
          ...
          $scope.uploadModel = function () {
            ...
            if (fileToUpload != undefined) {
              ...
              upload('./api/v1/importers?targetNamespace='
                  + $scope.targetNamespace.name, fileToUpload);
          ...
          upload = function (url, fileToUpload) {
            ...
            .then(
                function (result) {
                  ...
                  if ($scope.uploadResult.reports.length > 0
                      && $scope.uploadResult.valid) {
                    angular.forEach($scope.uploadResult.reports,
                        function (resultObject, idx) {
                          var item = (idx == 0) ? {active: false}
                              : {active: true};
                          ...
                          $scope.showCheckin = (resultObject.valid
                              && $scope.showCheckin);
                          $scope.fileAdded = !(resultObject.valid
                              && $scope.showCheckin);
                        });
                  } else {
                    $scope.showCheckin = false;
          ...
          $scope.isMissing = function (reference, missingReferences) {
            var index;
            for (index = 0;
                missingReferences != null && index < missingReferences.length;
                index++) {
          ...
          checkinSingle = function (handleId) {
            var importUrl = "./api/v1/importers/" + handleId + "?key="
                + $scope.selectedImporter.key + "&targetNamespace="
                + $scope.targetNamespace.name;
          ...
          $scope.getSelectedImporterInfo = function (selectedImporter) {
            ...
            for (var i = 0; i < $scope.importers.length; i++) {
              if ($scope.importers[i].key
                  == selectedImporter) {
                return $scope.importers[i];
              }
            }
          };
