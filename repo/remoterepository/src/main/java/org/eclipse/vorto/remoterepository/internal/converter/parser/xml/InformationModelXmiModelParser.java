...
public class InformationModelXmiModelParser extends AbstractXPathModelParser {

	@Override
	ModelView generateModelView(Document dom) throws XPathExpressionException {
		ModelId modelId = ModelFactory.newModelId(
				ModelType.INFORMATIONMODEL, namespaceExpr.evaluate(dom), versionExpr.evaluate(dom), nameExpr.evaluate(dom));

		return ModelFactory
				.newModelView(modelId, descriptionExpr.evaluate(dom));
	}

	@Override
	String getNameExpression() {
		return "/InformationModel/@name";
	}

	@Override
	String getNameSpaceExpression() {
		return "/InformationModel/@namespace";
	}

	@Override
	String getVersionExpression() {
		return "/InformationModel/@version";
	}

	@Override
	String getDescriptionExpression() {
		return "/InformationModel/@description";
	}

	@Override
	String getModelReferenceExpression() {
		return "/InformationModel/references";
	}
